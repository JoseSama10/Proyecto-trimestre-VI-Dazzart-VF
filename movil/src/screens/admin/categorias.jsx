...
export default function CategoriasAdmin() {
  ...
  const cargarCategorias = async () => {
    ...
    try {
      ...
      console.error("Error al listar:", err.message, err?.response?.data, err?.response?.status);
      Alert.alert(
        "Error",
  ...
  const guardarEdicion = async () => {
    ...
    if (!nombre || !descripcion) return Alert.alert("Error", "Completa todos los campos");
    ...
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: "#f0f2f5" }}>
        ...
        <FlatList
          ...
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={cargarCategorias} />}
          ListHeaderComponent={
          ...
          renderItem={({ item }) => (
            <View style={styles.card}>
              ...
              <View style={styles.cardActions}>
                ...
                <TouchableOpacity style={styles.deleteButton} onPress={() => eliminarCategoria(item.id_categoria)}>
  ...
  const styles = StyleSheet.create({
    ...
    addButton: { backgroundColor: "#ffc107", padding: 14, borderRadius: 10, alignItems: "center" },
